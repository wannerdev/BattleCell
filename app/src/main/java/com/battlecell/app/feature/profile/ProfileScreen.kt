        ...
        Row(
            ...
            Box {
                OutlinedButton(
                    ...
                    Text(text = stringResource(id = R.string.profile_allocate_more_button))
                }
                DropdownMenu(
                    expanded = dropdownExpanded,
                    onDismissRequest = { dropdownExpanded = false }
                ) {
                    listOf(3, 5, 10).forEach { option ->
                        DropdownMenuItem(
                            text = { Text(stringResource(id = R.string.profile_allocate_option, option)) },
                            onClick = {
                                dropdownExpanded = false
                                onAllocate(option)
                            },
                            enabled = option <= totalAvailable
                        )
                    }
                    DropdownMenuItem(
                        text = { Text(stringResource(id = R.string.profile_allocate_all_option, totalAvailable)) },
                        onClick = {
                            dropdownExpanded = false
                            onAllocate(totalAvailable)
                        },
